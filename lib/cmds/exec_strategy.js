...
const createSignalTracer = require('../util/create_signal_tracer')
...
module.exports = async (ds, ws, msg) => {
  ...
  const { id, label } = strategyContent
  const tracer = createSignalTracer(ds.dataDir, { label, symbol, tf })
  ...
  if (includeCandles) {
    ...
    tracer.createSignal('data_candles', startSignal, {
      key,
      start,
      end,
      orderBy,
  ...
  if (includeTrades) {
    ...
    tracer.createSignal('data_trades', startSignal, {
      key,
      start,
      end,
      orderBy,
  ...
  const execSignal = tracer.createSignal('execute', startSignal)
  ...
  execOffline(strategy, {
    ...
    execSignal.meta.results = results

    send(ws, ['bt.btresult', { ...data, ...results }, meta])
  }).finally(() => {
    return tracer.close()
  })
}
